...
useEffect(() => {
  ...
  const monTrongNgay = (Array.isArray(lichTuan) ? lichTuan : []).filter(
    (mon) =>
      parseInt(mon.daystart.slice(0, 2)) === day &&
      (isActive % 10) + 2 === parseInt(mon.thu)
  );
            ...
            <View style={styles.header}>
              ...
              <TouchableOpacity onPress={toggle} style={styles.monthTextWrapper}>
                <View style={styles.monthSelector}>
                  ...
                  <Text style={styles.dayOfWeekText}>{isActive === parseInt(`2${rday}`) ? "Hôm nay" : dateNamesToShow[isActive % 10]}</Text>
            ...
            >
              {weeks.map((week, index) => (
                <View key={index} style={styles.weekHeader}>
                  {week.map((d, i) => {
                    const dayNum = parseInt(d.slice(0, 2));
                    const weekdayNum = i + 2; 
                    const count = (Array.isArray(lichTuan) ? lichTuan : []).filter(
                      (mon) =>
                        parseInt(mon.daystart.slice(0, 2)) === dayNum &&
                        parseInt(mon.thu) === weekdayNum
                    ).length;
              ...
              <View style={{minHeight: height * 0.5, width: "100%", paddingVertical: 10}}>
                {monTrongNgay.map((mon, index) => (
                  <View key={index} style={{width: "100%"}}>
                    {parseInt(mon.daystart.slice(0,2)) === day && (isActive % 10) + 2 === parseInt(mon.thu) 
                    && <View style={{width: "100%"}}>
                      <TouchableOpacity onPress={() => handlePress(mon.link)} >
                        <View style={[
                          styles.subItem, 
                          mon.isTamNgung && {backgroundColor: "rgb(240,0,0,0.15)", opacity: 0.7}
                          ]}>
                          ...
                          {mon.isTamNgung && <View style={styles.tamNgung}><Text style={{fontFamily: "MuseoModerno", fontWeight: "500", fontSize: 14, color: "white", alignSelf: "center"}}>Tạm Ngưng</Text></View>}
                        </View>
                      </TouchableOpacity>
